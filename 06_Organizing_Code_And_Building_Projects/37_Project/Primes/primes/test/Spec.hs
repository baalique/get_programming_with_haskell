...
prop_validPrimesOnly val = if val < 2 || val >= length primes then isNothing result else isJust result
...
prop_factorsMakeOriginal val = isNothing result || (product (fromJust result) == val) where result = primeFactors val
