...
sieve :: [Int] -> [Int]
sieve []       = []
sieve (x : xs) = x : sieve noFactors where noFactors = filter ((/= 0) . (`mod` x)) xs
...
unsafePrimeFactors n (next : primes) =
    if n `mod` next == 0 then next : unsafePrimeFactors (n `div` next) (next : primes) else unsafePrimeFactors n primes
